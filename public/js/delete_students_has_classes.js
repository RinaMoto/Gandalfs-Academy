...
function deleteStudentsHasClasses(studentsHasClassesID) {
    ...
    xhttp.open("DELETE", "/delete-students_has_classes-ajax", true);
...
function deleteRow(studentsHasClassesID){
    ...
    let table = document.getElementById("students_has_classes-table");
    console.log(table);
    for (let i = 0, row; row = table.rows[i]; i++) {
       ...
       if (table.rows[i].getAttribute("data-value") == studentsHasClassesID) {
            table.deleteRow(i);
            deleteDropDownMenu(studentsHasClassesID);
            break;
       }
    }
